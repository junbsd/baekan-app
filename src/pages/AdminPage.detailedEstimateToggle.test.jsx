...
test("세부견적서 권한이 꺼져있는 사용자는 토글이 꺼진 상태(off)로 보인다", async () => {
  mockUsers([
    { id: "uid-1", name: "김철수", email: "b@test.com", role: "private", createdAt: "2026-01-01" },
  ]);
  render(<AdminPage user={adminUser} />);

  const toggle = getToggleByLabel("김철수", "🧾 세부견적서 사용 허용");
  expect(toggle).toHaveAttribute("aria-checked", "false");
});

test("토글을 켜면 canUseDetailedEstimate가 true로 저장된다", async () => {
  mockUsers([
    { id: "uid-1", name: "김철수", email: "b@test.com", role: "private", canUseDetailedEstimate: false, createdAt: "2026-01-01" },
  ]);
  ...
  expect(savedData.canUseDetailedEstimate).toBe(true);
});

test("이미 켜져있는 사용자의 토글을 누르면 false로 꺼진다", async () => {
  mockUsers([
    { id: "uid-1", name: "김철수", email: "b@test.com", role: "private", canUseDetailedEstimate: true, createdAt: "2026-01-01" },
  ]);
  const user = userEvent.setup();
  render(<AdminPage user={adminUser} />);
  ...
  await waitFor(() => expect(updateDoc).toHaveBeenCalled());
  const [, savedData] = updateDoc.mock.calls[0];
  expect(savedData.canUseDetailedEstimate).toBe(false);
});

test("관리자 본인 카드에는 세부견적서 토글이 표시되지 않는다(항상 사용 가능하므로)", async () => {
  mockUsers([
  ...
  expect(screen.queryByText("🧾 세부견적서 사용 허용")).not.toBeInTheDocument();
});
