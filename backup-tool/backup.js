...
async function runBackup() {
  ...
  // 폴더 생성
  ['', 'photos', 'videos', 'data', 'signatures'].forEach(sub =>
  ...
  // ── 4-1. opinions(소견서) 백업 ──────────────────────────────
  log('소견서 데이터 불러오는 중...');
  const opSnap = await db.collection('opinions').orderBy('date','desc').get();
  const opinions = opSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  log(`소견서 ${opinions.length}건`);

  fs.writeFileSync(path.join(backupDir,'data','opinions.json'), JSON.stringify(opinions,null,2),'utf8');
  const opHeader = '날짜,발급대상,소견유형,점검위치,현장,점검내용,소견,조치내용,작성자,작성자업체,사진수,작성일';
  const opRows = opinions.map(o=>[
    csvCell(fmtDate(o.date)), csvCell(o.issuedTo), csvCell(o.issueType), csvCell(o.location),
    csvCell(o.clientCompany), csvCell(o.findings), csvCell(o.opinion), csvCell(o.recommendation),
    csvCell(o.workerName), csvCell(o.workerCompany), csvCell((o.photoUrls||[]).length), csvCell(o.createdAt),
  ].join(','));
  fs.writeFileSync(path.join(backupDir,'data','opinions.csv'), '\uFEFF'+[opHeader,...opRows].join('\n'), 'utf8');
  log('opinions.json + opinions.csv 저장 완료');
  ...
  // ── 5-1. 서명(도장) 이미지 백업 ──────────────────────────────
  log('서명·도장 이미지 다운로드 중...');
  let sigOk=0, sigFail=0;
  for (const u of users) {
    if (!u.signatureUrl) continue;
    try {
      const dest = path.join(backupDir, 'signatures', `${u.name||u.id}_${u.id}.png`);
      if (!fs.existsSync(dest)) await downloadFile(u.signatureUrl, dest);
      sigOk++;
    } catch(e) { sigFail++; }
  }
  log(`서명 이미지 — 성공:${sigOk} 실패:${sigFail}`);
...
data/opinions.csv     ← 엑셀에서 바로 열기 가능
data/opinions.json    ← 전체 소견서 데이터
...
signatures/           ← 작업자 서명·도장 이미지 ${sigOk}개
`;
